...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run Column 04 Donchian/Turtle breakout tutorial.")
    ...
    p.add_argument("--allow-short", action="store_true", default=False, help="Allow symmetric short breakouts.")
...
def main() -> int:
    ...
    classical = run_classical_breakout_baselines(ohlcv, allow_short=args.allow_short, fee_bps=args.fee_bps, slippage_bps=args.slippage_bps)
    detime = run_detime_breakout_baselines(ohlcv, features, allow_short=args.allow_short, fee_bps=args.fee_bps, slippage_bps=args.slippage_bps)
    ...
    weights = {}
    weights.update(make_classic_breakout_weight_grid(ohlcv, allow_short=args.allow_short))
    weights.update(make_detime_breakout_weight_grid(ohlcv, features, allow_short=args.allow_short))
    turnover_report(weights).to_csv(report_dir / "column_04_turnover_report.csv")
    ...
    write_run_manifest(
        ...
        command="python examples/quant_trading/scripts/run_column_04_breakout_volume_confirmation.py",
        dataset="market_ohlcv",
        strategies=list(comparison.index),
        result_file=str(report_dir / "column_04_strategy_comparison.csv"),
    )
    print("Column 04 finished")
