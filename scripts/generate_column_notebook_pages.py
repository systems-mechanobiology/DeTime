...
NOTEBOOK_INDEX_INTROS = {
    "quant-trading": (
        "These pages render the executed Quant Trading tutorial notebooks, "
...
QUANT_TUTORIAL_NAV = [
    ("Roadmap", "00_decomposition_first_quant_trading_roadmap", "Tutorial 00 - Roadmap"),
    ("Strategy Lab", "01_detime_trend_following_strategy_lab", "01 Trend-Following Lab"),
    ("Tutorial Sequence", "01_market_data_and_decomposition_feature_factory", "01 Real Market Data and Feature Factory"),
    ("Tutorial Sequence", "02_decomposition_aware_moving_average_macd", "02 Decomposition-aware MA and MACD"),
    ("Strategy Lab", "02_detime_oscillation_reversion_strategy_lab", "02 Oscillation-Reversion Lab"),
    ("Strategy Expansion", "03_detime_method_specific_strategy_variants", "03 Method-Specific Variants"),
    ("Tutorial Sequence", "03_residual_mean_reversion_rsi_bollinger", "03 Residual Mean Reversion"),
    ("Strategy Expansion", "04_detime_component_pair_trading_cointegration", "04 Component Pair Trading"),
...
def _tutorial_navigation(column: dict[str, Path | str], current_stem: str) -> list[str]:
    if str(column["key"]) != "quant-trading":
        return []
    ...
    for track, stem, label in QUANT_TUTORIAL_NAV:
        if stem == current_stem:
            link = f"**{label}**"
        else:
            link = f"[{label}]({stem}.md)"
        lines.append(f"| {track} | {link} |")
    lines.append("")
    return lines
...
def render_notebook_page(column: dict[str, Path | str], notebook_path: Path) -> tuple[Path, str]:
    ...
    lines = [
        ...
        f'  <strong>Executed tutorial notebook.</strong> This page is generated from <a href="{source_url}"><code>{source_rel}</code></a> and includes markdown cells, code cells, stdout, tables, and captured figures from the committed notebook.',
        "</div>",
        "",
        *_tutorial_navigation(column, notebook_path.stem),
        "## Executed Notebook",
        "",
    ]
